...
//	NVIC_InitTypeDef NVIC_InitStructure;
    ...
    HAL_GPIO_Init(GPIOA, &GPIO_InitStructure);//初始化GPIOA.9

    //USART1_RX	  GPIOA.10初始化
    GPIO_InitStructure.Pin = GPIO_PIN_10;//PA10
    GPIO_InitStructure.Mode = GPIO_MODE_AF_INPUT;
//          GPIO_Mode_IN_FLOATING;//浮空输入
//    GPIO_InitStructure(GPIOA, &GPIO_InitStructure);//初始化GPIOA.10
...
//	NVIC_Init(&NVIC_InitStructure);	//根据指定的参数初始化VIC寄存器
    ...
    USART_InitStructure.WordLength = USART_WORDLENGTH_8B;//字长为8位数据格式
    USART_InitStructure.StopBits = USART_STOPBITS_1;//一个停止位
    USART_InitStructure.Parity = USART_PARITY_NONE;//无奇偶校验位
//	USART_InitStructure. = USART_HardwareFlowControl_None;//无硬件数据流控制
    USART_InitStructure.Mode = USART_MODE_RX | USART_MODE_TX;    //收发模式

//    HAL_USART_Init(USART1, &USART_InitStructure); //初始化串口1
